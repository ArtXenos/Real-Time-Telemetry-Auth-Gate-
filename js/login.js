...
const passwordLength = document.getElementById("counter");
let passLength = passwordInput.value.length;
...
loginForm.addEventListener("submit", function (event) {
  ...
  if (username === "admin" && password === "secret123") {
    ...
    passwordInput.value = "";
    passwordInput.focus();
    passwordLength.textContent = `Character Counter: 0`
  }
});

passwordInput.addEventListener("input", function () {
    passLength = passwordInput.value.length;
    passwordLength.textContent = `Character Counter: ${passLength}`
});
